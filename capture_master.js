...
async function main() {
    const profileDir = path.join(process.env.TEMP, 'chrome_master_strict_' + Date.now());
    ...
    try {
        ...
        await page.captureFullPage('01a-landing-page-hero.png', 'Smart Healthcare', 'index.php');

        // 01b: Landing Page Features Grid
        await page.eval(`window.scrollTo(0, 450);`);
        await new Promise(r => setTimeout(r, 300));
        await page.captureFullPage('01b-landing-page-faq-expanded.png', 'Everything You Need', 'index.php');
        ...
        await page.captureFullPage('02d-register-validation-error.png', 'Passwords do not match', 'register.php');
        ...
        await page.captureFullPage('04a-forgot-password-default.png', 'Reset Password', 'forgot-password.php');
        ...
        `);
        await new Promise(r => setTimeout(r, 300));
        await page.captureFullPage('04b-forgot-password-token-generated.png', 'Reset Password', 'forgot-password.php');
        ...
        await page.captureFullPage('07a-symptom-assessment-empty.png', 'Step 1: Symptom Assessment', 'book-appointment.php');
        ...
        await page.eval(`
            ...
            const ta = document.getElementById('symptoms_text') || document.querySelector('textarea');
            if (ta) ta.value = 'Experiencing dry coughing, continuous low fever, and fatigue for 2 days.';
        `);
        await new Promise(r => setTimeout(r, 200));
        await page.captureFullPage('07b-symptom-assessment-filled.png', 'Experiencing dry coughing', 'book-appointment.php');

        // 08a: Diagnosis Normal Result
        await page.eval(`document.querySelector('form').submit();`);
        await new Promise(r => setTimeout(r, 1400));
        await page.captureFullPage('08a-diagnosis-normal-result.png', 'Preliminary Diagnostic Result', 'appointment-result.php');
        ...
        await page.eval(`
            ...
            cbs.forEach(cb => {
                ...
                if (label.includes('chest') || label.includes('breath') || label.includes('shortness')) cb.checked = true;
            });
            const ta = document.getElementById('symptoms_text') || document.querySelector('textarea');
            if (ta) ta.value = 'Severe sudden crushing chest pain radiating to left arm and acute shortness of breath.';
            document.querySelector('form').submit();
        `);
        await new Promise(r => setTimeout(r, 1400));
        await page.captureFullPage('08b-diagnosis-emergency-result.png', 'EMERGENCY', 'appointment-result.php');
        ...
        `);
        await new Promise(r => setTimeout(r, 1200));
        await page.captureFullPage('10a-appointment-step2-doctor-select.png', 'Step 2: Choose Doctor & Time', 'book-appointment.php');
        ...
        await page.captureFullPage('10b-appointment-step2-time-picker.png', 'Step 2: Choose Doctor & Time', 'book-appointment.php');
        ...
        await page.captureFullPage('11a-appointment-conflict-working-hours.png', 'Step 2: Choose Doctor & Time', 'book-appointment.php');

        // 11b: Conflict 5-min Buffer
        await page.eval(`
            const dateInput = document.getElementById('appointment_date');
            const timeInput = document.getElementById('appointment_time');
            if (dateInput) dateInput.value = '2026-08-20';
            if (timeInput) {
                timeInput.value = '14:02';
                timeInput.dispatchEvent(new Event('change'));
                timeInput.dispatchEvent(new Event('input'));
            }
        `);
        await new Promise(r => setTimeout(r, 800));
        await page.captureFullPage('11b-appointment-conflict-5min-buffer.png', 'Step 2: Choose Doctor & Time', 'book-appointment.php');
        ...
        await page.goto('http://localhost/healthcare/reschedule-appointment.php?appointment_id=1');
        ...
        await page.captureFullPage('21a-printable-receipt-paid.png', 'MEDICAL INVOICE', 'view-receipt.php');
        ...
        await page.captureFullPage('16a-doctor-appointments-queue.png', 'Doctor Appointments', 'doctor-appointments.php');
        ...
        await page.goto('http://localhost/healthcare/doctor-appointments.php');
        await page.captureFullPage('17a-doctor-completion-time-guard.png', 'Mark as Completed', 'doctor-appointments.php');

        // 18a: Doctor Profile
        await page.goto('http://localhost/healthcare/doctor-profile.php');
        await page.captureFullPage('18a-doctor-profile-form.png', 'Specialization', 'doctor-profile.php');

        // 19a: Billing Generation Empty
        await page.goto('http://localhost/healthcare/billing.php');
        await page.captureFullPage('19a-billing-generation-empty.png', 'Generate Patient Bill', 'billing.php');
        ...
        await page.captureFullPage('19b-billing-generation-calculated.png', 'Generate Patient Bill', 'billing.php');
        ...
        // 23a: Admin Panel Users Tab (Patients table overview)
        await page.goto('http://localhost/healthcare/admin-panel.php');
        await page.captureFullPage('23a-admin-panel-users-tab.png', 'All Patients', 'admin-panel.php');

        // 23b: Admin Panel Search Filtered
        await page.eval(`
            const search = document.querySelector('input[placeholder*="Search patients"]');
            if (search) {
                search.value = 'Nasib';
        ...
        `);
        await new Promise(r => setTimeout(r, 400));
        await page.captureFullPage('23b-admin-panel-search-filtered.png', 'Nasib Rehman', 'admin-panel.php');

        // 23c: Admin Panel Patients Table
        await page.goto('http://localhost/healthcare/admin-panel.php');
        await page.eval(`window.scrollTo(0, 150);`);
        await new Promise(r => setTimeout(r, 400));
        await page.captureFullPage('23c-admin-panel-patients-tab.png', 'All Patients', 'admin-panel.php');

        // 23d: Admin Panel Doctors Table
        await page.eval(`
            const docHeader = Array.from(document.querySelectorAll('.card-header')).find(el => el.textContent.includes('Doctors'));
            if (docHeader) docHeader.scrollIntoView();
        `);
        await new Promise(r => setTimeout(r, 400));
        await page.captureFullPage('23d-admin-panel-doctors-tab.png', 'All Doctors', 'admin-panel.php');

        // 23e: Admin Panel Appointments Table
        await page.eval(`
            const apptHeader = Array.from(document.querySelectorAll('.card-header')).find(el => el.textContent.includes('Appointments'));
            if (apptHeader) apptHeader.scrollIntoView();
        `);
        await new Promise(r => setTimeout(r, 400));
        await page.captureFullPage('23e-admin-panel-appointments-tab.png', 'All Appointments', 'admin-panel.php');

        // 24a: Admin Suspend User Action
        await page.goto('http://localhost/healthcare/admin-panel.php');
        await page.captureFullPage('24a-admin-suspend-user-action.png', 'Suspend', 'admin-panel.php');
